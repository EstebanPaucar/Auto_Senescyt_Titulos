...
def procesar_excel_concurrente(archivo_bytes):
    ...
    for fila in range(3, ws.max_row + 1):
        ...
        elif ced not in cedulas_en_cola:
            pendientes.append((fila, ced))
            cedulas_en_cola.add(ced)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for futuro in concurrent.futures.as_completed(futuros):
                try:
                    ...
                    # Escribir en la memoria del Excel para la primera fila encontrada
    ...
    # 🔄 Rellenar TODAS las filas múltiples usando el caché final antes de guardar
    print("🔄 Sincronizando filas múltiples con el caché final...")
    for fila in range(3, ws.max_row + 1):
        ced_cruda = ws.cell(row=fila, column=3).value
        if not ced_cruda: continue
        ced = ''.join(filter(str.isdigit, str(ced_cruda).split('.')[0])).zfill(10)[:10]
