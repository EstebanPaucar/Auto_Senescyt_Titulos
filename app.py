...
def encriptar(data):
    return Fernet(st.secrets["ENCRYPTION_KEY"]).encrypt(data)

def desencriptar(data):
    return Fernet(st.secrets["ENCRYPTION_KEY"]).decrypt(data)
...
def obtener_sha_archivo():
    ...
    try:
        return repo.get_contents("outputs/matriz_procesada.xlsx.enc").sha 
    except Exception:
        return None
...
with pestaña_scraping:
        ...
        if st.button("🚀 Enviar al Servidor de GitHub (5 Hilos)", key="btn_ejecutar_scraping", type="primary"):
            with st.spinner("Inyectando archivo en la cola de procesamiento..."):
                try:
                    ...
                    bytes_encriptados = encriptar(bytes_a_subir)
                    
                    subir_a_github(bytes_encriptados, "inputs/matriz_a_procesar.xlsx.enc", "⏳ Nuevo lote de docentes subido por Streamlit")
    ...
    if st.button("🔄 Buscar Archivo Final", key="btn_recuperar"):
        ...
        with st.status("⏳ Esperando a que el servidor de GitHub termine... (Esto tomará unos minutos)", expanded=True) as status:
            ...
            while not archivo_listo and intentos < 270:
                try:
                    g = Github(GH_TOKEN)
                    repo = g.get_repo(REPO_NAME)
                    contents = repo.get_contents("outputs/matriz_procesada.xlsx.enc")
                    sha_actual = contents.sha
                    
                    if sha_actual == sha_viejo:
                        raise Exception("Sigue siendo el archivo viejo.")
                    
                    bytes_encriptados = contents.decoded_content
                    bytes_resultado = desencriptar(bytes_encriptados) 
                    archivo_listo = True
            ...
            if not archivo_listo:
                st.error("⚠️ El servidor tardó demasiado o el proceso falló en GitHub Actions. Revisa el caché y vuelve a intentarlo.")
                status.update(label="❌ Error: Tiempo de espera agotado", state="error", expanded=False)
            else:
